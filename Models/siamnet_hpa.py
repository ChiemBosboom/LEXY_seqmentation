...
Original file is located at
    https://colab.research.google.com/drive/1UGVAEtWg6nOGUGykcN1fLCKOYsf9V_Lk
"""
...
val_dataset = HPA(images_val, transform=test_transforms)
test_dataset = HPA(images_test, transform=test_transforms)
...
loss_type='bce'  # loss_type must be 'combined', 'dice', or 'bce'
loss_fn = DiceBCELoss(loss_type=loss_type)
model = VGG16_SiamNet().to(device)  # model_name must be 'SiamNet', 'VGG16_SiamNet', or 'ResNet34_SiamNet'
optimizer = torch.optim.Adam(params = model.parameters(), lr=0.0001)

# train model
train_loss_list, val_loss_list = fit_model(model, train_loader, val_loader, loss_fn, optimizer, epochs=8)
...
for i in range(0, rows * cols, 3):
    frame, mask, mask_prev, frame_transformed, binary_smoothed_mask = test_sample_batch[i]
    x1 = torch.stack([frame, mask_prev], dim=0).unsqueeze(0)
    x2 = torch.stack([frame_transformed, binary_smoothed_mask], dim=0).unsqueeze(0)

    # get model prediction
    pred = model(x1, x2).squeeze().squeeze().cpu().detach().numpy()
    pred = (pred >= 0.5).astype(np.uint8)

    # Display the cell image
    fig.add_subplot(rows, cols, i + 1)
    plt.imshow(frame.squeeze().cpu(), cmap='gray')
    plt.axis('off')
...
for i, seq in enumerate(tqdm(img_seq_val)):
    seq = HPA(seq, transform=test_transforms)
...
num_cells = len(results_val)

# Calculate sums for mean calculations
for cell in results_val:
    stats = results_val[cell]
