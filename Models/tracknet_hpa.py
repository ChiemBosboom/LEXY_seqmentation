...
Original file is located at
    https://colab.research.google.com/drive/1VtYb7Y5-3RshljqfhTqPBE4IX5VFvJxt
"""
...
class VGG16_TrackNet(nn.Module):
    def __init__(self):
        super().__init__()
...
class ResNet34_TrackNet(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    with torch.no_grad():
        for i, img in enumerate(images):
            if i == 0:
                ...
                # Extract input image (X_test) and target mask (y_test)
                X_test = img[[0, 2], :, :]
                X_test[1, :, :] = test_pred.to(torch.float32)
                y_test = img[1, :, :].to(torch.int)

            # Forward pass: Predict output mask using the model
            test_pred = model(X_test.unsqueeze(0)).squeeze()
...
def apply_transformation(img):
    ...
    return torch.tensor(np.stack([frame, mask, mask_prev], axis=0), dtype=torch.float32).to(device)
...
model = ResNet34_TrackNet().to(device)  # model_name must be 'TrackNet', 'VGG16_TrackNet', or 'ResNet34_TrackNet'
...
for i in range(0, rows * cols, 3):
    cell, mask, prev = test_sample_batch[i]

    # get model prediction
    pred = model(torch.stack([cell, prev], dim=0).unsqueeze(0)).squeeze().squeeze().cpu().detach().numpy()
